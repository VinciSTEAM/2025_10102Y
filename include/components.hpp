...
// motor groups
pros::MotorGroup leftMotors({7, -8, -6},
...
std::unordered_map<std::string, std::shared_ptr<pros::Distance>> distances {
    ...
    {"front", std::make_shared<pros::Distance>(19)}};
ArmNamespace::Arm arm(std::make_shared<pros::Motor>(12, pros::v5::MotorGears::red), // arm - motor port 5 (reversed)
                      std::make_shared<pros::Rotation>(-10), // rotation sensor - port 17 (reversed)
                      armAngularController); // PID controller
SpinnerNamespace::Spinner intake(std::make_shared<pros::Motor>(20, pros::v5::MotorGears::green)); // intake - motor port 7
SpinnerNamespace::Spinner hooks(std::make_shared<pros::Motor>(18, pros::v5::MotorGears::green)); // hooks - motor port 11
ConveyorNamespace::Conveyor conveyor(&intake, // intake
                                     &hooks, // hooks
                                     std::make_shared<pros::Optical>(9), // optical sensor - port 14
                                     std::make_shared<pros::Distance>(5) // distance sensor - port 20
);
HolderNamespace::Holder holder(std::make_shared<pros::adi::DigitalOut>('E'),
                               std::make_shared<pros::Distance>(21)); // mobile goal holder - port 'B'
HolderNamespace::Holder doinker(std::make_shared<pros::adi::DigitalOut>('A')); // doinker - port 'A'
HolderNamespace::Holder hanger(std::make_shared<pros::adi::DigitalOut>('B')); // hanger - port 'H'
HolderNamespace::Holder intake_raiser(std::make_shared<pros::adi::DigitalOut>('F')); // intake peu - port 'C'
